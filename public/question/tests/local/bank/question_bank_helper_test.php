...
final class question_bank_helper_test extends \advanced_testcase {
    ...
    public function test_get_instances_with_limit_and_capabilities(): void {
        ...
        for ($i = 0; $i < 20; $i++) {
            ...
            if ($i >= 15) {
                role_assign($editingteacherroleid, $user->id, \context_module::instance($sharedmod->cmid));
            }
        }
        ...
        // Now test that the function will first check question banks in courses in which the user has role assignments
        // for the requested capabilities (will be sorted to the top).
