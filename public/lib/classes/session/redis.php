...
class redis extends handler implements SessionHandlerInterface {
     ...
     * It is meant to be used by the environment checks only, see {@see self::check_environment()}.
     *
     * @return string The version of the Redis server or the minimum version requirement if the server could not be reached
     *  or does not support the INFO command.
     */
    ...
    public function check_environment(\environment_results $result): ?\environment_results {
        ...
        $result->setStatus(version_compare($serverversion, self::REDIS_MIN_SERVER_VERSION, '>='));
        $result->setCurrentVersion($serverversion);
        $result->setNeededVersion(self::REDIS_MIN_SERVER_VERSION);
