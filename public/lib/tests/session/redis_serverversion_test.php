...
#[CoversMethod(redis::class, 'check_server_version')]
...
final class redis_serverversion_test extends \advanced_testcase {
    ...
    /**
     * Test that the environment check is skipped if the Redis session handler is not in use.
     *
     * @param string|null $handlerclass The value of $CFG->session_handler_class, null if it is not configured.
     */
    #[DataProvider('check_server_version_not_in_use_provider')]
    public function test_check_server_version_not_in_use(?string $handlerclass): void {
        global $CFG;
        ...
        if ($handlerclass !== null) {
            $CFG->session_handler_class = $handlerclass;
        } else {
            unset($CFG->session_handler_class);
        }

        $this->assertNull(redis::check_server_version(new \environment_results('custom_check')));
    }

    /**
     * Data provider for {@see test_check_server_version_not_in_use()}.
     *
     * @return array[]
     */
    public static function check_server_version_not_in_use_provider(): array {
        return [
            'no_handler_configured' => [
                'handlerclass' => null,
            ],
            'other_handler_configured' => [
                'handlerclass' => '\core\session\file',
            ],
        ];
    }
}
